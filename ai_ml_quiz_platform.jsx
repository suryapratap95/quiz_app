...
export default function QuizApp() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const r = await window.storage.get("quiz_submissions");
        if (r && r.value) setSubmissions(JSON.parse(r.value));
      } catch {}
    })();
  }, []);
  ...
  const saveSubmission = async (sub) => {
    ...
    try { await window.storage.set("quiz_submissions", JSON.stringify(updated)); } catch {}
  };
  ...
  const submitQuiz = async () => {
    ...
    const sub = { email, setId: activeSet, setTitle: set.title, answers: { ...answers }, timestamp: new Date().toISOString() };
  ...
  const clearAll = async () => {
    if (confirm("Delete ALL student responses? This cannot be undone.")) {
      setSubmissions([]);
      try { await window.storage.delete("quiz_submissions"); } catch {}
    }
  };
        ...
        {view === "home" && (
          <div className="fade-up">
            ...
            <div style={{ background: "#fff", borderRadius: 16, padding: "28px 28px 24px", marginBottom: 24, border: "1px solid #e2e8f0", boxShadow: "0 2px 12px rgba(0,0,0,.04)" }}>
              <div style={{ fontSize: 13, fontWeight: 600, color: "#64748b", textTransform: "uppercase", letterSpacing: 1.5, marginBottom: 12 }}>Enter your email to begin</div>
              <input
                type="email" placeholder="yourname@email.com" value={email}
                onChange={e => { setEmail(e.target.value); setEmailError(""); }}
                style={{ width: "100%", padding: "14px 16px", borderRadius: 12, border: "2px solid " + (emailError ? "#ef4444" : "#e2e8f0"), fontSize: 15, fontFamily: font, background: "#fafbfc" }}
              />
              {emailError && <div style={{ color: "#ef4444", fontSize: 12, marginTop: 6, fontWeight: 500 }}>{emailError}</div>}
            </div>
                  ...
                  {setSubs.map((sub, si) => {
                    ...
                    return (
                      <div key={si} style={{ background: "#fff", borderRadius: 12, padding: "14px 20px", marginBottom: 8, border: "1px solid #e2e8f0", display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 8 }}>
                        <div>
                          <div style={{ fontSize: 14, fontWeight: 600, color: "#1e1b4b" }}>{sub.email}</div>
