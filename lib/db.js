  ...
  const { rows } = await query(
    `SELECT 1
     ...
     WHERE schemaname = 'public'
       AND indexname = 'idx_quiz_results_email_unique'
     LIMIT 1`
  );

  if (rows.length === 0) {
    await query(
      `CREATE UNIQUE INDEX idx_quiz_results_email_unique ON quiz_results (email)`
    );
  }
}
...
export async function ensureQuizTable() {
  ...
  try {
    ...
    console.error("quiz_results email unique index migration:", error?.message);
  }
