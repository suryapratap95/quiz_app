...
export async function ensureSchema() {
  ...
  await query(`ALTER TABLE exams ADD COLUMN IF NOT EXISTS parent_exam_id TEXT DEFAULT NULL`);
  await query(`ALTER TABLE exams ADD COLUMN IF NOT EXISTS set_label TEXT DEFAULT NULL`);
  ...
  await query(`
    ...
    END $$;
  `);
  // Some databases ended up with BOTH legacy (set_id/set_title) and new
  // (exam_id/exam_title) columns. Sync values, then drop the legacy columns so
  // inserts that only target exam_id no longer violate set_id NOT NULL.
  await query(`
    DO $$
    BEGIN
      IF EXISTS (
        ...
        UPDATE quiz_results
        SET exam_id = COALESCE(exam_id, set_id)
        WHERE exam_id IS NULL OR exam_id = '';

        UPDATE quiz_results
        SET exam_title = COALESCE(exam_title, set_title)
        WHERE exam_title IS NULL OR exam_title = '';

        ALTER TABLE quiz_results DROP COLUMN set_id;
      END IF;
      ...
      IF EXISTS (
        ...
        ALTER TABLE quiz_results DROP COLUMN set_title;
      END IF;
    END $$;
  `);
  await query(`
